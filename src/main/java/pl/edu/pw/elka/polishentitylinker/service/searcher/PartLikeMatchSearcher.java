...
public class PartLikeMatchSearcher implements Searcher {
    ...
    public List<WikiItemEntity> findCandidates(NamedEntity namedEntity) {
        ...
        while (enititySpan.size() > 0) {
            List<WikiItemEntity> candidates = wikiItemRepository.findAllByOriginalFormSimilarFromAliases(
                    TokenizedTextUtils.spanToOriginalForm(enititySpan));
